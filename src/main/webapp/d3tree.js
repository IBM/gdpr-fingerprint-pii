...
function update(source) {
  ...
  var nodeEnter = node.enter().append('g')
      ...
      .attr("transform", function(d) {
        return "translate(" + source.y0 + "," + source.x0 + ")"; // change for direction
    })
    .on('click', click);
  ...
  nodeEnter.append('text')
      .attr("dy", ".35em") // direction
      .attr("x", function(d) { // direction
  ...
  var nodeExit = node.exit().transition()
      .duration(duration)
      .attr("transform", function(d) {
          return "translate(" + source.y + "," + source.x + ")"; // direction
      })
      .remove();
  ...
  function diagonal(s, d) {
	  // chnage x and y for direction here also
    path = `M ${s.y} ${s.x}
            C ${(s.y + d.y) / 2} ${s.x},
              ${(s.y + d.y) / 2} ${d.x},
              ${d.y} ${d.x}`
